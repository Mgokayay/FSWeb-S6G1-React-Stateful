...
/* ADIM 1 */
  const[dondurucuAçık, setdöndürücüAcik]=useState(true);

  const toggleDondurucu = () => {
  setdöndürücüAcik(!dondurucuAçık)
  };
  ...
  return (
    <div className='widget-spinner container'>
      <h2>Döndürücü</h2>
      {
       dondurucuAçık  && <div id='döndürücü' className='spinner'>--+--</div> /* ADIM 2 */
      }
      <button id='toggleDondurucu' onClick={toggleDondurucu}>
          {dondurucuAçık ? "Gizle" : "Göster"}
      </button>
    </div>
  );
}
